...
int load_custom_bt_conf(struct btradio_conf_data *cfg)
{
    ...
    */
    ...
    if(retLen < sizeof(temp)){
        BT_ERR_FUNC("File read error len: %lu\n", retLen);
        return -1;
    }
    else{
...
int load_internal_bt_conf(struct btradio_conf_data *cfg)
{
    ...
    if (retLen < 0){
        ...
        if (written < 0){
            ...
            BT_ERR_FUNC("File write error len: %lu\n", written);
        }
        else{
...
void hci_stp_dev_init_rx_cb (const PUINT8 data, INT32 count)
{
    struct hci_stp *hu;
    unsigned int idx;
...
*/
void stp_rx_event_cb_directly(const PUINT8 data, INT32 count)
{
...
static int __init hci_stp_init(void)
{
    ...
    BT_DBG_FUNC("hci_get_drvdata(hdev) 0x%016lx\n", (unsigned long)hci_get_drvdata(hdev));
